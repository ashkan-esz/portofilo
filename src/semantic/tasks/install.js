  ...
  if (currentConfig && manager.name === 'NPM') {
    ...
    if (fs.existsSync(updatePaths.definition)) {
      ...
      if (currentConfig.version !== release.version) {
        ...
        console.info('Updating default themes...');
        ...
        // copy themes import
        console.info('Updating themes import file');
        gulp.src(source.themeImport)
          .pipe(plumber())
          .pipe(gulp.dest(updatePaths.themeImport))
        ;

        console.info('Adding new site themes files...');
...
// PM that supports Build Tools (NPM Only Now)
  ...
  gulp.task('create install files', function (callback) {
    ...
    if (manager && (answers.useRoot || answers.customRoot)) {
      ...
      // copy themes import
      console.info('Adding themes files');
      gulp.src(source.themeImport)
        .pipe(plumber())
    ...
    if (fs.existsSync(installPaths.site)) {
      ...
      console.info('Creating site themes folder', installPaths.site);
    }
    ...
    gulp.task('create themes.config', function () {
      var
        // determine path to site themes folder from themes config
      ...
      // rewrite site variable in themes.less
      console.info('Adjusting @siteFolder to: ', pathToSite + '/');

      if (fs.existsSync(installPaths.themeConfig)) {
        console.info('Modifying src/themes.config (LESS config)', installPaths.themeConfig);
        ...
        console.info('Creating src/themes.config (LESS config)', installPaths.themeConfig);
    ...
    gulp.task('create semantic.json', function () {
      ...
      // adjust variables in themes.less
    ...
    gulp.series('create themes.config', 'create semantic.json')(callback);
  });
