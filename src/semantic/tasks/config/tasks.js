        ...
        errorHandler: function(error) {
          ...
          if(error.filename.match(/theme.less/)) {
            if (error.line == 9) {
              ...
              if (element) {
                console.error('Missing themes.config value for ', element);
              }
              console.error('Most likely new UI was added in an update. You will need to add missing elements from themes.config.example');
            } else if (error.line == 73) {
              element = regExp.element.exec(error.message)[1];
              theme   = regExp.theme.exec(error.message)[1];
              console.error(theme + ' is not an available themes for ' + element);
            } else {
              console.error(error);
            }
          }
